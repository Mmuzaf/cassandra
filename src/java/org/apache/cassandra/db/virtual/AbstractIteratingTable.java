 ...
 */
public class AbstractIteratingTable<R> implements VirtualTable
{
    ...
    // todo fix to use the clock from the walker
    private final MonotonicClock clock = MonotonicClock.Global.approxTime;

    public AbstractIteratingTable(SystemView<R> systemView, UnaryOperator<String> tableNameMapper)
    {
    ...
    public static String virtualTableNameStyle(String camel)
    {
        ...
        return Arrays.stream(subNames)
                .map(AbstractIteratingTable::camelToSnakeWithAbbreviations)
                .reduce((a, b) -> a + '_' + b)
    ...
    private static TableMetadata createMetadata(SystemView<?> systemView, UnaryOperator<String> tableNameMapper)
    {
        ...
        systemView.walker().visitMeta(
                new RowWalker.MetadataVisitor()
                {
                    ...
                    {
                        // todo fix to use type insteand and the count method from walker
                        if (index == 0)
                        {
